# Be sure to update model path below to point to your local LLama3 8B model -- e.g. saved in ~/.cache/huggingface/hub/models--meta-llama--Meta-Llama-3-8B/snapshots/561487d18c41c76bcb5fc6cfb73a324982f04f47/"
...
hf_token=# Add huggingface token here
...
class LLMClassifier(nn.Module):
	...
	def __init__(self, model_path, num_labels, device_map=None, device=None, tokenizer_path=None):
		super().__init__()
		...
		self.llm = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.bfloat16, device_map=self.device_map, token=hf_token)

		print("LLM device", self.llm.device)
		...
		self.llm.config.pad_token_id=self.tokenizer.convert_tokens_to_ids("<|endoftext|>")
		...
		get_peft_model(self.llm, self.config)

		self.fc1 = nn.Linear(self.llm.config.hidden_size, num_labels).bfloat16()


	def forward(self, x):

		encoded_input = self.tokenizer(x, truncation=True, max_length=max_length, padding=True, return_tensors='pt').to(self.device)
		shape=encoded_input["input_ids"].shape
		encoded_inputIds = encoded_input["input_ids"].to(self.device)
		encoded_attnMask = encoded_input["attention_mask"].to(self.device)
		outputs = self.llm(input_ids=encoded_inputIds, attention_mask=encoded_attnMask, output_hidden_states=True, return_dict=True).hidden_states[-1]

		out1 = self.fc1(outputs)

		idx_of_last_tok=encoded_attnMask.bool().sum(1)-1

		sequence_lengths = torch.eq(encoded_input["input_ids"], self.llm.config.pad_token_id).int().argmax(-1) - 1
		sequence_lengths = sequence_lengths % shape[-1]
		sequence_lengths = sequence_lengths.to(self.device)

		batch_size = shape[0]

		embeds1 = out1[torch.arange(batch_size, device=self.device), sequence_lengths]
...
class LLMRegressor(nn.Module):
	...
	def __init__(self, model_path, max_length=None, device=None, device_map=None, tokenizer_path=None):
		...
		self.llm.config.pad_token_id=self.tokenizer.convert_tokens_to_ids(self.tokenizer.pad_token)
		...
		self.fc1 = nn.Linear(self.llm.config.hidden_size, 1).bfloat16()


	def forward(self, x):

		encoded_input = self.tokenizer(x, truncation=True, max_length=self.max_length, padding=True, return_tensors='pt').to(self.device)
		shape=encoded_input["input_ids"].shape
		encoded_inputIds = encoded_input["input_ids"].to(self.device)
		encoded_attnMask = encoded_input["attention_mask"].to(self.device)
		outputs = self.llm(input_ids=encoded_inputIds, attention_mask=encoded_attnMask, output_hidden_states=True, return_dict=True).hidden_states[-1]
...
def get_batches(all_x, all_y1, task, batch_size=4):
	...
	for i in range(0, len(all_x), batch_size):
		...
		if task == "regression":
			batches_y1.append(torch.FloatTensor(batch_y1).bfloat16())
...
class Objective:
	...
	def __call__(self, trial):
		...
		model_path="/PATH/TO/MODELS/.cache/huggingface/hub/models--meta-llama--Meta-Llama-3-8B/snapshots/561487d18c41c76bcb5fc6cfb73a324982f04f47/"
		...
		if self.task == "classification":
			...
			model=LLMClassifier(model_path, num_labels, device_map=device_map, device=device).to(device)
			...
			dev_y=self.dev_y
			test_y=self.test_y
			model=LLMRegressor(model_path, max_length=self.max_length, device_map=self.device_map, device=self.device).to(self.device)
			loss_fn=nn.MSELoss()

		batch_x, batch_y = get_batches(self.train_x, train_y, task, batch_size=4)
		dev_batch_x, dev_batch_y = get_batches(self.dev_x, dev_y, task, batch_size=4)
		test_batch_x, test_batch_y = get_batches(self.test_x, test_y, task, batch_size=4)
